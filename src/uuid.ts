...
export function enableDeterminsticIDs() {
  deterministic = true;
}

export function disbaleDeterminsticIDs() {
  deterministic = false;
}
export function resetDeterminsticIDCounter() {
  deterministicID = 0;
}

export default function uuid(): string {
  if (deterministic) {
    if (deterministicID >= determinsticValues.length) {
      throw new Error('Ran out of determinstic IDs');
    }
    return determinsticValues[deterministicID++];
  }
